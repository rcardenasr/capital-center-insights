...
const routes = {
  ...
  alerts() {
    return `${head("Monitoreo", "Centro de alertas", `<button class="btn" data-action="notify">Permitir notificaciones</button><button class="btn wine" data-action="simulate">Simular alerta</button>`)}<section class="card pad"><div class="table-wrap"><table><thead><tr><th>Prioridad</th><th>Novedad</th><th>Cliente</th><th>Fuente</th><th>Fecha</th><th>Accion</th></tr></thead><tbody>${alerts().map((a) => `<tr><td>${pill(a.priority, cls(a.priority))}</td><td><strong>${sourceAnchor(a, h(a.title))}</strong><div class="muted">${h(a.summary)}</div></td><td>${h(a.clientName)}</td><td>${sourceAnchor(a, h(a.source))}</td><td>${a.date}</td><td>${a.url ? `<a class="btn" href="${h(a.url)}" target="_blank" rel="noopener">Fuente</a>` : `<button class="btn" data-client="${a.clientId}">Abrir</button>`}</td></tr>`).join("")}</tbody></table></div></section>`;
  },
  settings() {
...
function alertCard(a) { return `<article class="alert-item"><div class="alert-head"><strong>${sourceAnchor(a, h(a.title))}</strong>${pill(`${a.priority} - ${a.score}`, cls(a.priority))}</div><span class="muted">${h(a.summary)}</span><div class="pill-row">${pill(a.type)}${a.url ? sourceAnchor(a, pill(a.source)) : pill(a.source)}${pill(a.date)}</div></article>`; }
function bind() {
